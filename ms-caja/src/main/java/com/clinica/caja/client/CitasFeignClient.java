...
import com.clinica.caja.client.dto.CitaResponseDTO;
...
public interface CitasFeignClient {
    ...
    ResponseEntity<CitaResponseDTO> actualizarEstado(@PathVariable Long id,
                                                     @RequestBody EstadoCitaUpdateDTO request);

    @PostMapping("/citas/{id}/compensar-pago-fallido")
    ResponseEntity<CitaResponseDTO> compensarPagoFallido(@PathVariable Long id);
}
