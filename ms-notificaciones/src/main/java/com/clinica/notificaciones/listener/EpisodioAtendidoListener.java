...
public class EpisodioAtendidoListener {
    ...
    public void onEpisodioAtendido(Message message, EpisodioAtendidoEvent evento) {
        ...
        log.info("ARCH_LOG PROCESSING queue={} idCita={} correlationId={}",
                RabbitMQConfig.QUEUE_EPISODIO_ATENDIDO, evento.getIdCita(), cid);
        ...
        CompletableFuture.runAsync(() -> {
            try { auditoriaClient.registrar(AccionAuditoriaDTO.builder()
                    ...
                    .entidadTipo("EpisodioAtendido").entidadId(String.valueOf(evento.getIdCita()))
                    .resultado("EXITO").correlationId(cidRef).build(), null);
            } catch (Exception ignored) {}
        });

        try {
            String resultado = notificacionService.notificarEpisodioAtendido(evento);
            ...
            log.info("ARCH_LOG {} queue={} idCita={} correlationId={}",
                    resultado, RabbitMQConfig.QUEUE_EPISODIO_ATENDIDO, evento.getIdCita(), cid);
        } catch (Exception ex) {
            ...
            log.error("ARCH_LOG ERROR queue={} idCita={} correlationId={} error={}",
                    RabbitMQConfig.QUEUE_EPISODIO_ATENDIDO, evento.getIdCita(), cid, ex.getMessage());
            throw ex;
        } finally {
            MDC.clear();
        }
