...
public class PagoConsultaConfirmadoListener {
    ...
    public void onPagoConsultaConfirmado(Message message, PagoConsultaConfirmadoEvent evento) {
        ...
        log.info("ARCH_LOG PROCESSING queue={} idCita={} correlationId={}",
                RabbitMQConfig.QUEUE_PAGO_CONFIRMADO, evento.getIdCita(), cid);
        ...
        CompletableFuture.runAsync(() -> {
            try { auditoriaClient.registrar(AccionAuditoriaDTO.builder()
                    ...
                    .entidadTipo("PagoConsultaConfirmado").entidadId(String.valueOf(evento.getIdCita()))
                    .resultado("EXITO").correlationId(cidRef).build(), null);
            } catch (Exception ignored) {}
        });

        try {
            String resultado = notificacionService.notificarPagoConfirmado(evento);
            ...
            log.info("ARCH_LOG {} queue={} idCita={} correlationId={}",
                    resultado, RabbitMQConfig.QUEUE_PAGO_CONFIRMADO, evento.getIdCita(), cid);
        } catch (Exception ex) {
            ...
            log.error("ARCH_LOG ERROR queue={} idCita={} correlationId={} error={}",
                    RabbitMQConfig.QUEUE_PAGO_CONFIRMADO, evento.getIdCita(), cid, ex.getMessage());
            throw ex;
        } finally {
            MDC.clear();
        }
