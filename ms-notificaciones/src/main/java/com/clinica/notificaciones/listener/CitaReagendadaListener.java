...
public class CitaReagendadaListener {
    ...
    public void onCitaReagendada(Message message, CitaReagendadaEvent evento) {
        ...
        log.info("ARCH_LOG PROCESSING queue={} idCita={} correlationId={}",
                RabbitMQConfig.QUEUE_CITA_REAGENDADA, evento.getIdCita(), cid);
        ...
        CompletableFuture.runAsync(() -> {
            try { auditoriaClient.registrar(AccionAuditoriaDTO.builder()
                    ...
                    .entidadTipo("CitaReagendada").entidadId(String.valueOf(evento.getIdCita()))
                    .resultado("EXITO").correlationId(cidRef).build(), null);
            } catch (Exception ignored) {}
        });

        try {
            String resultado = notificacionService.notificarCitaReagendada(evento);
            ...
            log.info("ARCH_LOG {} queue={} idCita={} correlationId={}",
                    resultado, RabbitMQConfig.QUEUE_CITA_REAGENDADA, evento.getIdCita(), cid);
        } catch (Exception ex) {
            ...
            log.error("ARCH_LOG ERROR queue={} idCita={} correlationId={} error={}",
                    RabbitMQConfig.QUEUE_CITA_REAGENDADA, evento.getIdCita(), cid, ex.getMessage());
            throw ex;
        } finally {
            MDC.clear();
        }
